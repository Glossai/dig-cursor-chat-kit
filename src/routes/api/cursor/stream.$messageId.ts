    ...
    handlers: {
      GET: async ({ request, params }) => {
        ...
        const { data: run, error } = await supabaseAdmin
          .from("cursor_runs")
          .select(
            "id, user_id, agent_name, cursor_agent_id, cursor_run_id, assistant_message_id, model_id, last_event_id",
          )
          .eq("assistant_message_id", params.messageId)
        ...
        if (!upstream.ok || !upstream.body)
          return new Response(`Cursor stream unavailable (${upstream.status})`, {
            status: upstream.status === 410 ? 409 : 502,
          });
        ...
        const output = new ReadableStream({
          async start(controller) {
            ...
            try {
              ...
              await supabaseAdmin
                .from("cursor_messages")
                .update({ content: fullText, status: "complete", completed_at: completedAt })
                .eq("id", run.assistant_message_id);
              await supabaseAdmin
                .from("cursor_runs")
                .update({
                  ...
                  input_tokens: accounting?.usage.inputTokens,
                  output_tokens: accounting?.usage.outputTokens,
                  cache_read_tokens: accounting?.usage.cacheReadTokens,
                  cache_write_tokens: accounting?.usage.cacheWriteTokens,
                  total_tokens: accounting?.usage.totalTokens,
                  input_cost_micros: accounting?.cost.inputCostMicros,
                  output_cost_micros: accounting?.cost.outputCostMicros,
                  cache_read_cost_micros: accounting?.cost.cacheReadCostMicros,
                  cache_write_cost_micros: accounting?.cost.cacheWriteCostMicros,
                  total_cost_micros: accounting?.cost.totalCostMicros,
                  cost_source: accounting?.cost.source ?? "unavailable",
                  pricing_version: accounting?.cost.pricingVersion,
                  provider_usage: accounting?.raw as Json | undefined,
                  provider_cost: accounting?.cost.providerCost as Json | undefined,
                })
                .eq("id", run.id);
              ...
              await supabaseAdmin
                ...
                .update({
                  content: fullText,
                  status: "error",
                  error_code: "cursor_stream_failed",
