...
const textFromContent = (content: readonly unknown[]) => content.map((part) => typeof part === "object" && part && "type" in part && part.type === "text" && "text" in part ? String(part.text) : "").join("");
...
export function useCursorRuntime(threadId: string, messages: CursorMessage[]) {
  ...
  const initialMessages = useMemo<ThreadMessageLike[]>(() => messages.map((message) => ({
    id: message.id,
    role: message.role,
    content: [{ type: "text" as const, text: message.content }],
    status: message.status === "error" ? { type: "incomplete" as const, reason: "error" as const, error: message.error_message ?? "Cursor failed" } : { type: "complete" as const, reason: "stop" as const },
  ...
  const adapter = useMemo<ChatModelAdapter>(() => ({
    async *run({ messages: runtimeMessages, abortSignal }) {
      ...
      let responseText = "";
      const onAbort = () => { void cancel({ data: { assistantMessageId: started.assistantMessageId } }).catch(() => undefined); };
