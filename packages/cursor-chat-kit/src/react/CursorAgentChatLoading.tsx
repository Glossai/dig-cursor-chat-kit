...
export type CursorAgentChatLoadingProps = {
  className?: string;
  sidebar?: boolean;
};

export function CursorAgentChatLoading({ className, sidebar = true }: CursorAgentChatLoadingProps) {
  return (
    <SidebarProvider className={className} defaultOpen={sidebar}>
      {sidebar && (
      ...
      <SidebarInset className="min-h-svh overflow-hidden bg-background">
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          ...
          <div className="mx-auto w-full max-w-3xl px-4 pb-4">
            <Skeleton className={cn("h-14 w-full", "rounded-[28px]")} />
          </div>
        </div>
      </SidebarInset>
    </SidebarProvider>
