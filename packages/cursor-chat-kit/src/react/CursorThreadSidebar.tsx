...
export function CursorThreadSidebar({ agentName, threadId, labels, classNames, features, onSelectThread }: SidebarProps) {
  ...
  return <Sidebar collapsible="icon" className={cn("border-r border-sidebar-border", classNames.sidebar)}>
    ...
    <SidebarContent className="p-2">{!collapsed && <div className="mb-2 space-y-2"><div className="relative"><Search className="absolute left-2.5 top-2.5 size-3.5 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search threads" className="h-8 pl-8" /></div><Button variant="ghost" size="sm" className="w-full justify-start" onClick={() => setArchived((value) => !value)}><Archive />{archived ? "Active threads" : "Archived"}</Button></div>}{sections.map((section) => <div key={section.id} className={collapsed || openSections[section.id] ? "mb-2" : "mb-0"}>{!collapsed && <Button variant="ghost" size="sm" className={`${openSections[section.id] ? "mb-1" : "mb-0"} w-full justify-start px-2 text-xs font-medium text-muted-foreground`} onClick={() => toggleSection(section.id)} aria-expanded={openSections[section.id]}>{openSections[section.id] ? <ChevronDown /> : <ChevronRight />}{section.label}<span className="ml-auto tabular-nums">{section.threads.length}</span></Button>}{(collapsed || openSections[section.id]) && <SidebarMenu>{section.threads.map((thread) => <SidebarMenuItem key={thread.id} className="group/item flex items-center">
